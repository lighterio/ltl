...
var ltl = module.exports = (function () {
	...
	return {
		...
		compile: function (code, options) {
			...
			// If we end up in a dot block, remember the starting indent and filter.
			var blockIndent = 0;
			var blockFilter = '';
			var blockLines = [];
			...
			for (var i = 0; i < lines.length; i++) {
				...
				if (blockIndent) {
					...
					// If we're still in the block, append to the block code.
						...
						line = line.substring(Math.min(spaces, tabWidth * blockIndent));
					...
					while (rest && (++t < maxT)) {
						...
						while (rest && (++t < maxT)) {
							...
							// If the next character is a colon, enter a block.
								...
								blockFilter = trim(rest.substring(1));
								blockLines = [];
								rest = '';
								break;
							}
			...
			// If there's a path specified, cache the template in the module at that path.
			if (options.path) {
				this.templates[options.path] = template;
			}
