...
var ltl = module.exports = (function () {
	...
	var commandPattern = /^(use|get|set)\b/;
	...
	var ltl = {
		...
		compile: function (code, options) {
			...
			function appendBlock() {
				...
				// If we're in a "use" block, compile the contents.
				if (blockFilter == 'use') {
					appendText('html', "'+this['" + blockKey + "'].call(this," + contextVar + (text ? ',' + ltl.compile(text) : '') + ")+'");
					return;
				}
				// For a "set" block.
				else if (blockFilter == 'set') {
					blockSets.push("'" + escapeSingleQuotes(blockKey) + "':" + ltl.compile(text).toString());
					return;
				}
			...
			for (var i = 0; i < lines.length; i++) {
				...
				// Expression patterns make things append.
					...
					blockKey = pair[1];
					if (command == 'get') {
						appendText('html', "'+" + partsVar + "['" + blockKey + "'](" + contextVar + ")+'");
						hasGets = true;
					}
					else {
						startBlock(command);
			...
			// If there's a key specified, cache the template with that key.
			if (options.key) {
				this.cache[options.key] = template;
			}
